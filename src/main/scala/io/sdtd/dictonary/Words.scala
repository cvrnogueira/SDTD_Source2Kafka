...
object Words {
  ...
  */
  ...
  val depression = List("suicide", "depressed", "kill myself").map(word => word.toLowerCase())

  val depressionConcat = depression.mkString(",")
  val diseasesConcat = diseases.mkString(",")

  val allWords = diseases ++ depression
}
